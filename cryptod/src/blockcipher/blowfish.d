...
module cryptod.blockcipher.blowfish;
...
class Blowfish : BlockCipher
{
	...
	this(ubyte[] Key)
	{
		...
		this.Key = Key;
		for(uint k = 0; k < P.length;)
		{
			for(uint i = 0; i < Key.length/4 && i+k < P.length; i++)
			{
				lP[i+k] = lP[i+k] ^ cast(uint)(((Key[4*i]<<24)+(Key[4*i+1]<<16)+(Key[4*i+2]<<8)+(Key[4*i+3]<<0)));
			}
			k+=Key.length/4;
		}	
		...
		for(uint i = 0; i < sbox0.length/2; i++)
		{
			T = Cipher(T);
			lsbox0[2*i] = T[0];
			lsbox0[2*i+1] = T[1];
		}
		for(uint i = 0; i < sbox1.length/2; i++)
		{
			T = Cipher(T);
			lsbox1[2*i] = T[0];
			lsbox1[2*i+1] = T[1];
		}
		for(uint i = 0; i < sbox2.length/2; i++)
		{
			T = Cipher(T);
			lsbox2[2*i] = T[0];
			lsbox2[2*i+1] = T[1];
		}
		for(uint i = 0; i < sbox3.length/2; i++)
		{
			T = Cipher(T);
			lsbox3[2*i] = T[0];
			lsbox3[2*i+1] = T[1];
